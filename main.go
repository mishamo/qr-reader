	...
	loginBtn := widget.NewButton("Sign in with Google", func() {
		go func() {
			if err := a.authManager.Authenticate(); err != nil {
				// Check if this is a mobile auth error that needs code entry
...
func (a *QRScannerApp) showMobileAuthDialog() {
	...
	instructions := widget.NewLabel("Complete these steps:\n\n1. Sign in to Google in your browser\n2. Approve app permissions\n3. You'll see a page with an authorization code\n4. Copy the code and paste it below")
	instructions.Wrapping = fyne.TextWrapWord
